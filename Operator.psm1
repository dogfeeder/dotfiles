...
function Write-Theme {
    ...
    $path = $pathSymbol + " " + "$(Split-Path -path $pwd -Leaf)"
    ...
    if ($host.UI.RawUI.CursorPosition.X + $timestamp.Length + 9 -lt $host.UI.RawUI.WindowSize.Width){
        ...
        $prompt += Write-Prompt "[" -ForegroundColor $sl.Colors.PromptBackgroundColor
        $prompt += Write-Prompt -Object "$($sl.PromptSymbols.ClockSymbol)" -ForegroundColor $sl.Colors.PromptBackgroundColor
...
$sl.Colors.PromptBackgroundColor = [System.ConsoleColor]::White
